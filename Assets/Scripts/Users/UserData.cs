...
namespace TrainingBuddy.Users
{
	public class UserData : MonoBehaviour
	{
		...
		public IEnumerator LoadUserData()
		{
		    ...
		    var DBTask = DatabaseManager.Instance.DbReference.Child("Users").Child(DatabaseManager.Instance.User.UserId).GetValueAsync();
		    ...
		    else
		    {
		        ...
		        ExperiencePoints.text = snapshot.Child("ExperiencePoints").Value + "/800 XP";
		    }
		}
		...
		public IEnumerator UpdateSkillPoints(int skillPoints)
		{
		    ...
		    var DBTask = DatabaseManager.Instance.DbReference.Child("Users").Child(DatabaseManager.Instance.User.UserId).Child("SkillPoints").SetValueAsync(skillPoints);
		...
		{
			//Set the currently logged in user deaths
			var DBTask = DatabaseManager.Instance.DbReference.Child("Users").Child(DatabaseManager.Instance.User.UserId).Child("AccelerationPoints").SetValueAsync(accelerationPoints);
		...
		{
			//Set the currently logged in user deaths
			var DBTask = DatabaseManager.Instance.DbReference.Child("Users").Child(DatabaseManager.Instance.User.UserId).Child("SpeedPoints").SetValueAsync(speedPoints);
		...
		{
			//Set the currently logged in user deaths
			var DBTask = DatabaseManager.Instance.DbReference.Child("Users").Child(DatabaseManager.Instance.User.UserId).Child("ExperiencePoints").SetValueAsync(experiencePoints);
